...
from app.commands import build_brag_url, weighted_pace_sec
from app.streak import MILESTONE_STEP, highest_milestone


# --- weighted_pace_sec (누적 평균 페이스 — /스트릭·/자랑 공용 단일 공식) -----

def test_weighted_pace_prefers_paired_totals():
    # 짝지은(거리·시간 동시 인식) 총시간/총거리 가중식이 단순 평균보다 우선.
    s = {"paired_dist": Decimal("10.00"), "paired_dur": 4000, "pace_avg": Decimal("999")}
    assert weighted_pace_sec(s) == 400.0


def test_weighted_pace_falls_back_to_avg():
    s = {"paired_dist": None, "paired_dur": None, "pace_avg": Decimal("415.5")}
    assert weighted_pace_sec(s) == 415.5


def test_weighted_pace_zero_paired_distance_falls_back():
    # 0 나눗셈 방지: paired 거리가 0이면 단순 평균 폴백.
    s = {"paired_dist": Decimal("0"), "paired_dur": 0, "pace_avg": Decimal("400")}
    assert weighted_pace_sec(s) == 400.0


def test_weighted_pace_none_when_no_data():
    assert weighted_pace_sec({}) is None
