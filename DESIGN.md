...
최종 갱신: 2026-06-24 (성장 추세 스파크라인 + /캘린더 추가)
...
## 6. 명령어 ([bot/app/commands.py](bot/app/commands.py)) — 길드 동기화 top-level 6개
| 명령 | 동작 | 노출 |
|---|---|---|
...
| `/캘린더 [month]` | 월 러닝 달력(달린 날 `*`) + 월간·이번 주 합계. 온디맨드(자동게시 아님) | 본인만 |
...
## 9. 검증 방법
- 단위 테스트: `docker compose run --rm bot python -m pytest -q` (현재 40건: 스트릭·재계산·OCR·차트).
