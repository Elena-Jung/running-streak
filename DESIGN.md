...
- **구현 금지**: 위기 알림, 주간/월간 리포트 자동 게시, 그래프 생성, 목표·배지, 대규모 확장 설계.
  · **의도적 예외 — 자랑 카드(2026-07-03 결정)**: `/자랑` 마일스톤 카드는 '배지 금지'의 표면과 닿지만, 그 금지의 취지는 **자동·푸시형 게이미피케이션(스케줄러·자동 알림/게시)** 배제다. 자랑 카드는 **옵트인·사용자 개시(pull, 슬래시 커맨드)·자동게시 및 스케줄러 없음·서버 저장 없음** 이라 성격이 다르므로 예외로 허용한다. 새 게임 루프·넛지(알림/독촉/점수경쟁)는 도입하지 않는다.
...
| `/자랑` | **현재 스트릭**이 마일스톤(10/25/50/100일) 이상이면 통계를 프래그먼트(#)에 담은 **자랑 카드** 링크 회신. 미달이면 안내만. 읽기 전용(저장 없음) | 본인만(ephemeral) |
...
## 9. 검증 방법
- 단위 테스트: `docker compose run --rm bot python -m pytest -q` (현재 88건: 스트릭·재계산·OCR·차트·config·이벤트핸들러·04시 경계·트레드밀 속도/역산·킬스위치·쿨다운·**자랑 카드(highest_milestone·build_brag_url)**).
